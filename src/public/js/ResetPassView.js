...
function updateDots() {
  ...
  document.getElementById('stepIndicator').style.display = (currentStep === 4) ? 'none' : 'flex';
}
...
function submitStep3() {
  ...
  const cp = document.getElementById('confirmPass').value;
  if (np !== cp) {
    document.getElementById('step3ErrorMsg').textContent = 'Passwords do not match.';
    document.getElementById('step3Error').classList.add('show');
    return;
  }
  if (np.length < 8) {
    document.getElementById('step3ErrorMsg').textContent = 'Password must be at least 8 characters.';
    document.getElementById('step3Error').classList.add('show');
    return;
  }
...
document.addEventListener('keydown', e => {
  if (e.key === 'Enter') {
    ...
    if (currentStep === 3) { const btn = document.getElementById('step3Btn'); if (!btn.disabled) submitStep3(); }
  }
});
